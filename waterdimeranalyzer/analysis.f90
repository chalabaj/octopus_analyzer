 ...
 if (err.EQ.1) then
    ...
    write(*,*)'Couldnt find requested file. File: ',errfile
 end if 
 STOP          
...
! 14 1 mol H2 diss, OH...OH bondedg
        ...
        do hh=3,Natoms,1   ! 1 and 2 are oxygen atoms
          ...
          else if ( dist(1,hh).lt.dist(2,hh).AND.dist(1,hh).lt.OH_bond_dist ) then   ! if not dissociated then where the hydrogen is? O1 or O2
 	           h_o1=h_o1+1
