! gfortran -o watmon_analyzer  analysis.f90 watmon_analyzer_fastpop.f90  -g -O3
 ...
 INTEGER                  :: diss_H,diss_molH  ! water dimer analysis variables 
...
!----------proces movie------------------------- 
 ...
 !       CALL system('rm nlines.txt')
        ...
        do igeom=1,Ngeoms,1
           ...
           call channels(dist,Natoms,channel,diss_H,diss_molH)

            write(111,5)time,step,channel,dist(1,2),dist(1,3),diss_H,diss_molH
5           format(1F9.4,2I8.1,2F16.8,2I8.1) 
        end do  
...
!-----------READING DATAALL    
    ...
    do istep=1,outlines-1,1  !through entire file except first line
        read(112,*)time,step,channel,dist(1,2),dist(1,3),diss_H,diss_molH
